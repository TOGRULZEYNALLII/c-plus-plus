...
public:
  virtual void checkout() 
   ...
   virtual void checkin() 
    ...
    LibraryItem(const string &x, int &y, bool status) : title(x), year(y), status(status) {};
    ~LibraryItem() = default;
};
...
public:
    ...
    Book(const string &title, int year, const string &author, bool status)
        : LibraryItem(title, year,status), author(author) {}
};
...
public:
    ...
    {
        LibraryItem::displayInfo();
        cout << "The format of the e-book: " << format << endl;
    }
    EBook(const string &title, int year, const string &x, bool status) : LibraryItem(title, year,status), format(x) {};
...
public:
    ...
    {
        LibraryItem::displayInfo();
        cout << "The artist of the audio CD: " << artist << endl;
    }
  

    AudioCD(const string &title, int year, const string &x, bool status) : LibraryItem(title, year,status), artist(x) {};
};
...
{
    /*      Example usage:     */
    Book book1("The Great Gatsby", 1925, "F. Scott Fitzgerald", 1);
    EBook ebook1("The Hitchhiker's Guide to the Galaxy", 1979, "EPUB", 0);
    AudioCD audioCD1("Abbey Road", 1969, "The Beatles", 1);
    LibraryItem *libraryitems[] = {&book1, &ebook1, &audioCD1};
    for (int i = 0; i < sizeof(libraryitems) / sizeof(libraryitems[0]); i++)
    {
