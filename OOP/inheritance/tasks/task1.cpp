...
class Teacher : public Person {
   public:
    ...
    void ReviewGrades() const {
        cout << "Teacher "<<name << " is reviewing grades." << endl;
        cout << "Grades: " << GetGrades() << endl;
    }
};

int main() {
    Teacher teacher0("Mr. Smith", 40, 95);
    /*      Example usage:     */
    Teacher teacher1("Ms. Johnson", 35, 5);
    Teacher teacher2("Student Alice", 20, 90);
    teacher1.ReviewGrades();  // Accesses grades directly for review
    teacher2.ReviewGrades();  // Accesses grades directly for review
    teacher0.ReviewGrades();  // Accesses grades directly for review
